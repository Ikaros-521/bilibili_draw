    ...
    xmlhttp.onreadystatechange = function()
    {
        if(xmlhttp.readyState == 4)
        {
            if(xmlhttp.status == 200)
            {
                ...
                comment = json["data"]["item"]["modules"]["module_stat"]["comment"]["count"];
                var type = json["data"]["item"]["basic"]["comment_type"];
                //oid = json["data"]["card"]["desc"]["rid"];
                //comment = json["data"]["card"]["desc"]["comment"];
                //var type = json["data"]["card"]["desc"]["type"];
                if(type == "11") have_pic = 1;
                else have_pic = 0;
                console.log("oid=" + oid);
...
function oid_init()
{
    ...
    {
        var referer2 = referer.split("?");
        dynamic_id = referer2[0].replace(/[^0-9]/ig,"");
